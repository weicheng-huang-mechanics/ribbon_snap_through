...
void display(void)
{
	while ( myWorld.simulationRunning() > 0)
	{
		...
		{
			glVertex3f( myWorld.getBoundaryCoordination_left(4*i), myWorld.getBoundaryCoordination_left(4*i+1), myWorld.getBoundaryCoordination_left(4*i+2));
			glVertex3f( myWorld.getBoundaryCoordination_left(4*(i+1)), myWorld.getBoundaryCoordination_left(4*(i+1)+1), myWorld.getBoundaryCoordination_left(4*(i+1)+2));
		}
		glEnd();
		...
		for (int i = 0; i < NPTS-2; i++)
		{
			glVertex3f( myWorld.getBoundaryCoordination_right(4*i), myWorld.getBoundaryCoordination_right(4*i+1), myWorld.getBoundaryCoordination_right(4*i+2));
			glVertex3f( myWorld.getBoundaryCoordination_right(4*(i+1)), myWorld.getBoundaryCoordination_right(4*(i+1)+1), myWorld.getBoundaryCoordination_right(4*(i+1)+2));
		}
		glEnd();
		...
		glPointSize(5.0);
		glBegin(GL_POINTS);
		glVertex3f( myWorld.getScaledCoordinate(4*((NPTS-1))/2), myWorld.getScaledCoordinate(4*((NPTS-1))/2+1), myWorld.getScaledCoordinate(4*((NPTS-1))/2+2) );
		glEnd();
