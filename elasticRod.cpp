...
elasticRod::elasticRod(MatrixXd initialNodes, MatrixXd undeformed, 
	...
	double m_youngM, double m_shearM, double m_rodLength, VectorXd m_theta, double m_width)
{
...
{	
	...
	crossSectionalArea = rodRadius * width;
...
void elasticRod::setVertexOneConstraint(double position, int i, int k)
{
...
void elasticRod::setMass()
{
	...
	for (int i=0; i<nv; i++)
	{
		...
		if (i < nv-1)
		{
			massArray[4*i+3] = width * width * dm / 12.0;
		}
	}
}
...
void elasticRod::computeElasticStiffness()
{
	...
	*/
	...
	EI = youngM * width * rodRadius * rodRadius * rodRadius / 12;
	EA = youngM * width * rodRadius;
	GJ = 1.4562 * youngM * width * rodRadius * rodRadius * rodRadius / 12;
}
...
void elasticRod::getDiscreteTwist()
{
	for (int i=0; i < ne; i++)
    {
        theta_f(i) = x(4*i+3);
    ...
    for (int i = 1; i < ne; i++)
    {
    	tau(i) = theta_f(i) - theta_e(i) + refTwist(i) - undeformedTwist(i);
    } 
}
